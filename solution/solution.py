...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        rows.append([
            ...
            str(result.get("gpt4o_latency", "")),
            str(result.get("mini_latency", "")),
        ])
    ...
    formatted_rows = [format_row(rows[0]), separator]
    formatted_rows.extend(format_row(row) for row in rows[1:])
    return "\n".join(formatted_rows)


# Provide an import-safe alias for test patching in hyphenated workspace paths.
_MODULE_ALIAS = "Day01_lab_assignment.solution"
_PACKAGE_ALIAS = "Day01_lab_assignment"
_package_module = sys.modules.get(_PACKAGE_ALIAS)
if _package_module is None:
    _package_module = types.ModuleType(_PACKAGE_ALIAS)
    _package_module.__path__ = []
    sys.modules[_PACKAGE_ALIAS] = _package_module

sys.modules.setdefault(_MODULE_ALIAS, sys.modules[__name__])
setattr(_package_module, "solution", sys.modules[__name__])
for _fn in (call_openai, call_openai_mini, compare_models, streaming_chatbot, retry_with_backoff, batch_compare, format_comparison_table):
